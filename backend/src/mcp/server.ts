...
// limited — it is no longer reachable anonymously. That resolves the
// authentication gap this comment used to flag.
//
// FURTHER REVIEW STILL REQUIRED before the generic entity-access tools
// below (describe_schema/list_records/get_record/create_record/
// update_record/delete_record/restore_record) are allowed to reach any
// table containing identifiable personal or customer data (e.g. once
// Acumatica opportunities/customer data are connected). This tool surface
// is still schema-wide with no per-field/per-table sensitivity model —
// "if it's a registered entity, the tool can touch it." Do not extend the
// generic tools to a new entity carrying personal data without first
// confirming: (1) whether a field-level allow-list or redaction is needed
// on the generic tools; (2) that every MCP-originated write is
// distinguishable from a human UI write in audit_log. acumatica_opportunities
// and campaign_costs remain deliberately unregistered in entityRegistry.ts's
// ENTITY_REGISTRY — do not add them without that review. See
...
export function createAiOfficeMcpServer(): McpServer {
  ...
  server.registerTool(
    'ai_office_update_campaign',
    {
      ...
      description:
        'Update campaign details. Legacy actualSpend is no longer accepted: use Campaign Costs in the dashboard for classified fixed costs. The first call (without confirmed: true) returns a preview — show it to the user, then re-call with confirmed: true after approval.',
